...
export class ViewDocumentComponent implements OnInit, OnDestroy {
	 ...
	 */
	constructor(
		private _masterDocsComponent: MasterDocsComponent,
	...
	ngOnInit(): void {
		this._activatedRoute.paramMap.subscribe((params) => {
			...
			this._ngZone.run(() => {
				this.document = this._masterDocsComponent.items.find(
					(item) => item.fisiereDocumente.identifier === this.documentId
				);
				this._cdr.markForCheck();
			});
			this._masterDocsComponent.matDrawer.open();
	...
	closeDrawer(): Promise<MatDrawerToggleResult> {
		return this._masterDocsComponent.matDrawer.close();
	}
	downloadFile(): void {
