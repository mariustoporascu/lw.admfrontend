...
export class ViewDocumentComponent implements OnInit, OnDestroy {
	 ...
	 */
	constructor(
		private _masterDocsPreAppComponent: MasterDocsPreAppComponent,
	...
	ngOnInit(): void {
		this._activatedRoute.paramMap.subscribe((params) => {
			...
			this._ngZone.run(() => {
				this.document = this._masterDocsPreAppComponent.items.find(
					(item) => item.fisiereDocumente.identifier === this.documentId
				);
				this._cdr.markForCheck();
			});
			this._masterDocsPreAppComponent.matDrawer.open();
	...
	closeDrawer(): Promise<MatDrawerToggleResult> {
		return this._masterDocsPreAppComponent.matDrawer.close();
	}
	downloadFile(): void {
