...
export class CanDeactivateSearchForUser
	implements CanDeactivate<SearchForUserComponent>
{
	canDeactivate(
		...
		// file manager app
		if (!nextState.url.includes('operations')) {
			// Let it navigate
			return true;
		}

		// If we are navigating to another item...
		if (nextState.url.includes('/search-user')) {
			// Just navigate
			return true;
		}
		// Otherwise...
